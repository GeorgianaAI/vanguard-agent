...
- **Rate limiting** — Upstash Redis sliding-window counters enforce per-IP mission rate limits (5/rolling minute, 5/rolling hour) and per-IP approval rate limits; all counters checked before any AI pipeline runs
...
| **Rate-limit bypass / budget drain**           | Medium     | Medium    | Dual-bucket Redis sliding-window (5/min, 5/hr per IP); approval bucket separate      | Accepted — two-layer ceiling; hard cap before AI pipeline                                           |
...
| **LLM04** | **Model Denial of Service**          | **Strong.** Dual-bucket Redis rate limiting (5/min, 5/hr per IP for missions; separate bucket for approvals) runs before any AI pipeline. Circuit breaker terminates at 10 agent iterations. Edge function timeout provides a secondary wall-clock bound. _Gap: no per-mission token budget enforcement — a complex target with many tool calls increases token consumption per mission._                                                                              |
